...
&& isset($_POST['quantity'])&& isset($_POST['price'])&& isset($_POST['time'])&& isset($_POST['description'])
	...
	move_uploaded_file($_FILES["image"]["tmp_name"], $target_file);
	 ...
	 $sql = "INSERT INTO events (start, end, color, service, quantity, price, time, description, status, user_id, user_username, productid, image, animal ,user_name , user_phone , user_email) 
	 VALUES ('$start', '$end', '$color', '$service', '$quantity', '$price', '$time', '$description', '$status', '$user_id', '$user_username', '$productid', '$target_file', '$animal', '$user_name', '$user_phone', '$user_email')";
	// ตรวจสอบคิวซ้ำโดยเช็คในฐานข้อมูล
    $sql_check = "SELECT COUNT(*) FROM events WHERE time = :time AND start >= :start AND end <= :end AND productid = $productid";
	...
	header("Location: http://localhost/DLX/status.php");
		...
		<div class="container ">
			...
			<div class="row">
				...
				<div class="col-lg-12 text-center" >
					<h1>แบบฟอร์มกรอกข้อมูลใช้บริการ</h1>
					<a href="http://localhost/DLX/status.php" class="btn btn-warning mt-3">แก้ไขข้อมูล</a>
					<a href="http://localhost/DLX/service.php" class="btn btn-danger mt-3">ย้อนกลับ</a>
						...
						<div class="modal-content " style="margin-top: 40px;">
							...
							<input type="hidden" name="id" value="<?php  echo $row['id'];?>">
								...
								<div class="modal-body">
									...
									<div class="modal-body">
									<div class="form-group">
										<label for="title" class="col-sm-2 control-label">ราคา </label>
										<div class="col-sm-10">
										<input type="text" class="form-control" id="price" name="price" value="<?=$row['price']?>" readonly>
										</div>
									</div>


									<div class="form-group">
										<label for="description" class="col-sm-2 control-label">ข้อมูลสัตว์เพิ่มเติม</label>
										<div class="col-sm-10">
											<textarea type="text" name="description" class="form-control" id="description" rows="3" placeholder="กรุณากรอกข้อมูลสัตว์" required></textarea>
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="date" name="start" class="form-control" id="start" min="<?php echo date('Y-m-d'); ?>">
										</div>
									</div>
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="date" name="end" class="form-control" id="end" readonly>
										</div>
									</div>
									...
									<script>
										document.addEventListener("DOMContentLoaded", function() {
											...
											startDateInput.addEventListener("change", function() {
												// เมื่อผู้ใช้เปลี่ยนค่า "วันที่เริ่มใช้บริการ"
												// ตั้งค่าค่า "วันที่สิ้นสุดการใช้บริการ" ให้เท่ากับค่า "วันที่เริ่มใช้บริการ"
									...
									<div class="form-group">
										<label for="color" class="col-sm-2 control-label"required>เลือกสีปฎิทิน</label>
										<div class="col-sm-10">
									...
									<script>
										function checkUpload() {
											...
											if (imageInput.files.length > 0) {
												// มีการอัปโหลดไฟล์รูป
												confirmButton.disabled = false; // เปิดใช้งานปุ่มยืนยัน
											} else {
												// ไม่มีการอัปโหลดไฟล์รูป
												confirmButton.disabled = true; // ปิดใช้งานปุ่มยืนยัน
											}
										}
									</script>
								</div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        quantityInput.addEventListener("change", function() {
            // เมื่อผู้ใช้เลือกจำนวนสัตว์
            updatePrice();
        });

        function updatePrice() {
            var quantity = quantityInput.value; // รับค่าจำนวนสัตว์ที่เลือก
            var pricePerAnimal = 300; // ราคาต่อสัตว์
            var totalPrice = quantity * pricePerAnimal; // คำนวณราคารวม

            // อัพเดตค่าใน input ของราคา
